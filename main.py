...
def api_page_image(bid: str, n: int, w: int = 1100):
    load_meta(bid)
    w = max(300, min(int(w), 1800))
    with _render_lock:
        doc = _get_doc(bid)
        if n < 1 or n > doc.page_count:
            raise HTTPException(404, "Page out of range")
        page = doc[n - 1]
        zoom = w / max(1.0, page.rect.width)
...
function renderToc(){
  ...
  $('toc').innerHTML=rows.length?rows.map(e=>{
    ...
    return `<div class="toc-row lv${lv} indent${lv}">
      <span class="t-title" onclick="openReader(${e.start},${e.start},${e.end},'${esc(e.title).replace(/'/g,"&#39;")}',true)">${esc(e.title)}</span>
...
async function ftSearch(){
  ...
  $('ftHits').innerHTML=d.hits.length?d.hits.map(h=>
    `<div class="hit"><span class="pg">p. ${h.page}</span>${esc(h.snippet).replace(rx,m=>'<mark>'+m+'</mark>')}
      <button class="mini" onclick="openReader(${h.page},1,${cur.pages},'${esc(cur.name).replace(/'/g,"&#39;")}',false)">\uD83D\uDC41 open</button></div>`).join('')
