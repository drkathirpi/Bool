...
<body>
<header>
  <h1>BookSplit <small>read · split · ask · v__VER__</small></h1>
</header>
<div id="errbox" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:99;
  background:#7A1F1A;color:#fff;padding:10px 14px;font-size:13px;white-space:pre-wrap"></div>
<script>
window.addEventListener('error',e=>{
  const b=document.getElementById('errbox');b.style.display='block';
  b.textContent='JS error: '+e.message+' @ line '+e.lineno+' — screenshot this and send it';
});
window.addEventListener('unhandledrejection',e=>{
  const b=document.getElementById('errbox');b.style.display='block';
  b.textContent='Promise error: '+(e.reason&&e.reason.message||e.reason);
});
</script>
<main>
...
$('rImg').addEventListener('error',()=>{
  ...
  else{const b=document.getElementById('errbox');b.style.display='block';
    b.textContent='Page image failed after retries: '+$('rImg').src;}
});
...
def home():
    return HTMLResponse(PAGE.replace("__VER__", APP_VERSION),
